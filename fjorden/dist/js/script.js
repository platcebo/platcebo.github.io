...
let headerCartSlide = document.querySelectorAll('.header-cart__slide'),
    ...
    headerCartSlider = document.querySelectorAll('.header-cart__slider'),
    headerCartInput = document.querySelectorAll('.header-cart__info_check input');

if(headerCartCheck !== null && headerCartCheck.length !== 0) {
    headerCartInput[0].checked = true;
    headerCartSlider[0].classList.add('active')
    headerCartCheck.forEach((item, i)=>{
        item.addEventListener('click',()=>{
            for(let a = 0; a < headerCartCheck.length; a++) {
                headerCartSlider[a].classList.remove('active')
            }
            headerCartSlider[i].classList.add('active')

            if(document.querySelectorAll('.header-cart__slider.active .header-cart__slide').length < 2) {
                document.querySelector('.header-cart__slider.active .header-cart__func').classList.add('dis')
            }
        })
    })
...
if(videoBtn !== null) {
    ...
    document.body.addEventListener('click', (event)=>{
        ...
        if(target.classList.contains('modal-video__overflow') || target.closest('.modal-video__close')) {
            modalVideo.remove()
        } 
    })
}
...
document.body.addEventListener('click', function(event){
    ...
    if(target && target.classList.contains('project__pic')) {
        if(target.parentNode.classList.contains('hover')) {
