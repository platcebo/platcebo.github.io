...
function handleClick(event) {
    ...
    //  => не делаем ничего
    if (!catalog.contains(event.target) && !catalogBtn.contains(event.target)) {
        catalogBtn.classList.remove('new-nav__catalog_active');
        catalog.classList.remove('menu-tab__active');
        return;
    }
}
